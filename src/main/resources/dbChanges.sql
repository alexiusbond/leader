...
ALTER TABLE `spt`.`student_discount` 
ADD COLUMN `attachment_id` INT NULL DEFAULT NULL AFTER `discount_value`,
ADD INDEX `fk_student_discount_attachment1_idx` (`attachment_id` ASC) VISIBLE;
ALTER TABLE `spt`.`student_discount` ALTER INDEX `fk_student_discount_employee1_idx` INVISIBLE;
ALTER TABLE `spt`.`student_discount` 
ADD CONSTRAINT `fk_student_discount_attechment1`
  FOREIGN KEY (`attachment_id`)
  REFERENCES `spt`.`attachments` (`id`)
  ON DELETE RESTRICT
  ON UPDATE NO ACTION;
