...
import kg.alex.leader.domain.EmployeeBranchesExcel;
import kg.alex.leader.domain.EmployeeLessons;
import kg.alex.leader.excel.ExcelUploader;
import kg.alex.leader.i18n.Messages;
import kg.alex.leader.pdf.BranchCodesPdf;
        ...
        if (source == deleteBtn && yearSelect.getValue() != null) {
            ...
            try {
                myUI.getPage().open(new FileResource(
                                new File(Settings.PATH_TO_UPLOADS + "leader_hr_branches.xlsx")),
                        "_blank", false);
            } catch (Exception e) {
                logger.error(e);
                logger.catching(e);
