...
import kg.alex.muras.domain.EmployeeBranchesExcel;
import kg.alex.muras.domain.EmployeeLessons;
import kg.alex.muras.excel.ExcelUploader;
import kg.alex.muras.i18n.Messages;
import kg.alex.muras.pdf.BranchCodesPdf;
        ...
        if (source == deleteBtn && yearSelect.getValue() != null) {
            ...
            try {
                myUI.getPage().open(new FileResource(
                                new File(Settings.PATH_TO_UPLOADS + "muras_hr_branches.xlsx")),
                        "_blank", false);
            } catch (Exception e) {
                logger.error(e);
                logger.catching(e);
