...
public class ContractLiseWithOutDormitoryPdf_2025_ru {

    static final Logger logger = LogManager.getLogger(ContractLiseWithOutDormitoryPdf_2025_ru.class);
    ...
    public ContractLiseWithOutDormitoryPdf_2025_ru(final MyVaadinUI ui, StudentInfoPdf st_info, final IndexedContainer instPlanCont) {
        this.myUI = ui;
        ...
        StreamResource.StreamSource source1 = () -> {
            ...
            try {
                ...
                paragraph.clear();
                paragraph.setAlignment(Element.ALIGN_JUSTIFIED);
                paragraph.add(new Phrase("1.1. Предметом Договора является организация процесса обучения и воспитания Учащегося, получение им образования в рамках государственного образовательного стандарта и программ общеобразовательной средней школы, на период ", ordFont));
                ...
                paragraph.clear();
                paragraph.add(new Phrase("2.1.4. Обеспечить безопасность Учащегося во взаимодействии со структурными подразделениями Лицея, осуществляющими организацию внутри объектного и пропускного режимов.", ordFont));
                document.add(paragraph);
                ...
                paragraph.clear();
                paragraph.add(new Phrase("2.2.6. В случае пропуска учебных 45 дней по/без уважительной причины, Учащийся остается на повторный курс обучения (на второй год).", ordFont));
                document.add(paragraph);

                paragraph.clear();
                paragraph.add(new Phrase("2.2.7. В целях компетентного подхода к трудовому воспитанию, администрация Лицея имеет право привлекать учащихся к уборке своего рабочего места в классных комнатах.", ordFont));
                document.add(paragraph);

                paragraph.clear();
                paragraph.add(new Phrase("2.2.8. Лицей по желанию и согласованию с родителями, организовывает проведение подготовительных курсов к ОРТ, TOEFL, SAT, IELTS и т.д., кружковую работу по интересам учащихся, за отдельную плату. В связи с чем администрация Лицея имеет право запретить учащимся посещение дополнительных образовательных занятий вне Лицея.", ordFont));
                document.add(paragraph);

                paragraph.clear();
                paragraph.add(new Phrase("2.2.9. Учащимся, состоящим на диспансерном учете по хроническим заболеваниям (эпилепсии, астмы, порока сердца, энуреза и др.) особые условия не предоставляются. Родители, либо законные представители Учащихся, обязаны предупредить в письменном виде медицинского сотрудника Лицея о хронических заболеваниях Учащегося.", ordFont));
                document.add(paragraph);

                paragraph.clear();
                paragraph.add(new Phrase("2.2.10. Удержать ", ordFont));
                ...
                paragraph.clear();
                paragraph.add(new Phrase("2.2.11. При расторжении настоящего договора по непредвиденным обстоятельствам, с учетом всех понесенных расходов Лицея, сумма подлежащяя к возврату, возвращается по мере возможности Лицея, но не позднее мая следующего года.", ordFont));
                document.add(paragraph);

                paragraph.clear();
                paragraph.add(new Phrase("2.2.12. В целях оповещения образовательного процесса и деятельности Лицея без уведомления учащегося и родителей размещать фото и видеоматериалы в своих интернет страницах и СМИ.", ordFont));
                document.add(paragraph);

                paragraph.clear();
                paragraph.add(new Phrase("2.2.13. Лицей может иметь иные права, предусмотренные законодательством КР.", ordFont));
                document.add(paragraph);
                ...
                paragraph.clear();
                paragraph.add(new Phrase("2.4.2. Досрочно расторгнуть Договор с возмещением стоимости обучения в период пребывания Учащегося в Лицее в соответствии с п.2.2.9, 2.2.10.", ordFont));
                document.add(paragraph);
                document.add(new Paragraph(10, " "));
