...
public class IncomesExpensesReport implements Button.ClickListener,
    ...
    private void buildLeftPanel() {
        ...
        try {
            ...
            dbac.connect();
            dbac.execSQL_for_select_as_tree(myUI, "5", employeeCategoriesTable,
        ...
        if (source == generateBtn) {
            if (employeeCategoriesTable.getValue() != null && currencySelect.isValid() && fromDateDF.isValid() && tillDateDF.isValid()) {
                try {
                    ...
                    dataTable.setColumnAlignment(myUI.getMessage(SptMessages.Incomes), Table.Align.RIGHT);
                    dataTable.setColumnAlignment(myUI.getMessage(SptMessages.Expenses), Table.Align.RIGHT);
            ...
            try {
                EnhancedFormatExcelExport excelReport = new EnhancedFormatExcelExport(dataTable, myUI.getMessage(SptMessages.IncomesExpensesReport) + " ("
                        + currencySelect.getItemCaption(currencySelect.getValue()) + ")");
                excelReport.setReportTitle(myUI.getMessage(SptMessages.IncomesExpensesReport) + " ("
                        + currencySelect.getItemCaption(currencySelect.getValue()) + ") ");
