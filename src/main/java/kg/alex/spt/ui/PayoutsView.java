...
public class PayoutsView extends HorizontalSplitPanel implements Button.ClickListener,
    ...
    private String[] NATURAL_COL_ORDER, NATURAL_COL_ORDER_PAYOUTS;
    ...
    public void buttonClick(Button.ClickEvent event) {
        ...
        } else if (source.getId() != null && source.getId().equals(sysSettings.dbAcc_transactions)) {
    ...
    public void valueChange(Property.ValueChangeEvent event) {
        ...
        if (property == invoicesTable) {
            ...
            ((ComboBoxMax) payoutsTable.getContainerProperty(catCb.getId(), myUI.getMessage(SptMessages.Currency)).getValue()).setValue(
                    (Integer) catCb.getContainerProperty(catCb.getValue(), sysSettings.acc_currency_id).getValue());
            repaintPayoutsFooter();
    ...
    private void prepareNormalMode() {
        if (currentUser.isPermitted(sysSettings.cnPayoutsView + ":" + sysSettings.actModify)) {
            modifyBtn.setEnabled(true);
        }
        if (currentUser.isPermitted(sysSettings.cnPayoutsView + ":" + sysSettings.actAdd)) {
            createBtn.setEnabled(true);
        }
        if (currentUser.isPermitted(sysSettings.cnPayoutsView + ":" + sysSettings.actDelete)) {
            deleteBtn.setEnabled(true);
        }
        if (currentUser.isPermitted(sysSettings.cnPayoutsView + ":" + sysSettings.actCopy)) {
    ...
    private void updateDatacontainer() {
        ...
        try {
            invoicesTable.getContainerProperty(invoicesTable.getValue(), myUI.getMessage(SptMessages.Amount)).setValue(
                    sysSettings.dFormat.parse(payoutsTable.getColumnFooter(myUI.getMessage(SptMessages.Amount))).doubleValue());
        } catch (Exception e) {
            logger.error(e);
            logger.catching(e);
        }
    ...
    private void execDelete() {
        try {
            ...
            int st = dbDef.exec_delete(invoicesTable.getValue().toString(), sysSettings.dbAcc_transactions, sysSettings.db_acc_invoice_id);
            if (st != 0) {
                payoutsTable.removeAllItems();
                st = dbDef.exec_delete((Integer) invoicesTable.getValue(), sysSettings.dbAccInvoice);
                if (st != 0) {
    ...
    public IndexedContainer preparePayoutsContainer() {
        if (payoutsCont == null) {
            ...
            payoutsCont.addContainerProperty(sysSettings.button, Button.class, null);
            ...
            payoutsCont.addContainerProperty(sysSettings.acc_category_id, Integer.class, 0);
            payoutsCont.addContainerProperty(myUI.getMessage(SptMessages.Currency), ComboBoxMax.class, null);
            payoutsCont.addContainerProperty(sysSettings.acc_currency_id, Integer.class, 0);
            ...
            payoutsCont.addContainerProperty(sysSettings.crud_status, String.class, null);
        } else {
    ...
    private void addPayoutsItem() {
        NATURAL_COL_ORDER_PAYOUTS = new String[]{sysSettings.button,
        ...
        item.getItemProperty(sysSettings.button).setValue(
                createButton(myUI.getMessage(SptMessages.DeleteButton), id, sysSettings.dbAcc_transactions));
        ComboBoxMax cb = createCombobox(0, myUI.getMessage(SptMessages.Category), null, true, true);
        try {
            DbAccCategory dbCon = new DbAccCategory();
            dbCon.connect();
            cb.setContainerDataSource(dbCon.exec_for_select(myUI, 2, myUI.getUser().getSchool_id(), 0));
            dbCon.close();
        } catch (Exception e) {
            logger.error(e);
            logger.catching(e);
    ...
    private void repaintPayoutsFooter() {
        ...
        payoutsTable.setColumnFooter(myUI.getMessage(SptMessages.Amount), sysSettings.dFormat.format(totalAmount) + " $");
    }
        ...
        try {
            ...
            if (delPayoutsIds.size() > 0) {
                for (int i = 0; i < delPayoutsIds.size(); i++) {
                    dbd.exec_delete(delPayoutsIds.get(i), sysSettings.dbAcc_transactions);
                }
            }
            if (payoutsTable.getContainerDataSource().size() > 0) {
                Iterator iter = payoutsTable.getItemIds().iterator();
                while (iter.hasNext()) {
                    ...
                    tr.setFrom_to_employee_id((Integer) cb.getContainerProperty(cb.getValue(), sysSettings.employee_id).getValue());
                    if (payoutsTable.getContainerProperty(next, sysSettings.crud_status).getValue().toString().equals(myUI.getMessage(SptMessages.Update))) {
                        tr.setId(next.toString());
                        dbAt.exec_update(tr);
                    } else if (payoutsTable.getContainerProperty(next, sysSettings.crud_status).getValue().toString().equals(myUI.getMessage(SptMessages.Insert))) {
