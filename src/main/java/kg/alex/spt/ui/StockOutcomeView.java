...
public class StockOutcomeView extends HorizontalSplitPanel implements Button.ClickListener,
            ...
            try {
                ...
                if (st.getScl_accountent_fullname() != null) {
                    if (st.getScl_address() != null && st.getScl_phone() != null
                        ...
                        new StockMovementsPdf(myUI, myUI.getMessage(SptMessages.StockOutcome), stInv, movementsCont, st,
    ...
    public void valueChange(Property.ValueChangeEvent event) {
        ...
        if (property == invoicesTable) {
            ...
            if (((ComboBoxMax) movementsTable.getContainerProperty(changedItemId,
                ...
                try {
                    int cat_id = (Integer) ((ComboBoxMax) movementsTable.getContainerProperty(changedItemId,
                            myUI.getMessage(SptMessages.Product)).getValue()).getValue();
                    int msr_id = (Integer) ((ComboBoxMax) movementsTable.getContainerProperty(changedItemId,
                            myUI.getMessage(SptMessages.Measurement)).getValue()).getValue();
                    int inv_id = 0;
                    if (!isNew) {
                        inv_id = invID;
                    ...
                    if (movementsTable.getContainerDataSource().size() > 0) {
                        ...
                        ArrayList<StockMovement> list = dbCon.execSQL_remains(cat_id, msr_id, (Integer) stockSelect.getValue());
                        int i = 0;
                        while (iter.hasNext()) {
                            ...
                            if (((ComboBoxMax) movementsTable.getContainerProperty(next,
                                ...
                                if ((int) ((ComboBoxMax) movementsTable.getContainerProperty(changedItemId,
                                        myUI.getMessage(SptMessages.Product)).getValue()).getValue()
                                        == (int) ((ComboBoxMax) movementsTable.getContainerProperty(next,
                                        myUI.getMessage(SptMessages.Product)).getValue()).getValue()
                                        && (int) ((ComboBoxMax) movementsTable.getContainerProperty(changedItemId,
                                        myUI.getMessage(SptMessages.Measurement)).getValue()).getValue()
                                        == (int) ((ComboBoxMax) movementsTable.getContainerProperty(next,
                                    ...
                                    try {
                                        ...
                                        while (i < list.size() && quantity > 0.0) {
                                            if (quantity > list.get(i).getRemain()) {
                                                q += list.get(i).getRemain();
                                                avg_rate += list.get(i).getRate() * list.get(i).getRemain();
                                                avg_price += list.get(i).getPrice() * list.get(i).getRemain();
                                                amount += list.get(i).getPrice() * list.get(i).getRemain();
                                                quantity -= list.get(i).getRemain();
                                            } else {
                                                q += quantity;
                                                avg_rate += list.get(i).getRate() * quantity;
                                                avg_price += list.get(i).getPrice() * quantity;
                                                list.get(i).setRemain(list.get(i).getRemain() - quantity);
                                                amount += list.get(i).getPrice() * quantity;
                                                quantity = 0.0;
                                                i--;
                                            }
                                            i++;
                                        ...
                                        if (q != 0) {
                                            movementsTable.getItem(next).getItemProperty(myUI.getMessage(SptMessages.Rate)).setValue(avg_rate / q);
                                            movementsTable.getItem(next).getItemProperty(myUI.getMessage(SptMessages.Price)).setValue(avg_price / q);
                                            movementsTable.getItem(next).getItemProperty(myUI.getMessage(SptMessages.Amount)).setValue(amount);
                                        }
                                    } catch (Exception e) {
                                        logger.error(e);
                                        logger.catching(e);
            ...
            if (((AbstractField) property).getData() != null && ((AbstractField) property).getData().equals(myUI.getMessage(SptMessages.Quantity))) {
                ...
                if (movementsTable.getContainerProperty(changedItemId, myUI.getMessage(SptMessages.Price)).getValue() != null
    ...
    private void recalculateRemaindersAfterInsert(Object changedItemId) {
        ...
        while (iter.hasNext()) {
            ...
            if (((ComboBoxMax) movementsTable.getContainerProperty(next,
                ...
                if (!next.equals(changedItemId)
                        ...
                        && measurement_id == (Integer) ((ComboBoxMax) movementsTable.getContainerProperty(
                        next, myUI.getMessage(SptMessages.Measurement)).getValue()).getValue()) {
                    old_remainder = (Double) movementsTable.getContainerProperty(next, myUI.getMessage(SptMessages.Remain)).getValue();
        ...
        if (value_from_db == 0 && !isExistsSame) {
            ...
            ((TextField) movementsTable.getContainerProperty(changedItemId,
                    myUI.getMessage(SptMessages.Quantity)).getValue())
                    .addValidator(new DoubleRangeValidator(myUI.getMessage(SptMessages.NotifWrongValue), 0.1, 0.0));
    ...
    private void addMovementsItem() {
        ...
        item.getItemProperty(Settings.button).setValue(
                createButton(myUI.getMessage(SptMessages.DeleteButton), id, Settings.dbStockMovement));
        ...
        cb = createCombobox(0, myUI.getMessage(SptMessages.Measurement), Settings.dbMeasurement, true);
        cb.addValueChangeListener(this);
        cb.setId(id);
            ...
            if (movementsTable.getContainerDataSource().size() > 0) {
                ...
                while (iter.hasNext()) {
                    ...
                    smv.setAcc_category_id((Integer) ((ComboBoxMax) movementsTable.getItem(next).getItemProperty(
                            myUI.getMessage(SptMessages.Product)).getValue()).getValue());
                    smv.setMeasurement_id((Integer) ((ComboBoxMax) movementsTable.getItem(next).getItemProperty(
                            myUI.getMessage(SptMessages.Measurement)).getValue()).getValue());
                    ...
                    try {
                        ...
                        for (int i = 0; i < list.size() && quantity > 0.0; i++) {
                            smv.setQuantity(quantity);
                            if (quantity > list.get(i).getRemain()) {
                                smv.setQuantity(list.get(i).getRemain());
                                quantity -= list.get(i).getRemain();
                            } else {
                                quantity = 0.0;
                            }
                            smv.setPrice(list.get(i).getPrice());
                            smv.setStock_movement_id(list.get(i).getId());
                            smv.setRate(list.get(i).getRate());
    ...
    public Component getNewObj() {
        return new StockOutcomeView(myUI);
    }
}
